...
pmtrace_t *FindHullIntersection(float *vecSrc, float *vecEnd, pmtrace_t *tr, float *pflMins, float *pfkMaxs)
{
	...
	trTemp = pmove->PM_TraceLineEx(vecSrc, vecHullEnd, PM_NORMAL, 0, CL_TraceEntity_Ignore );
	...
	for (int i = 0; i < 2; i++)
	{
		for (int j = 0; j < 2; j++)
		{
			for (int k = 0; k < 2; k++)
			{
				...
				trTemp = pmove->PM_TraceLineEx(vecSrc, vecNewEnd, PM_NORMAL, 0, CL_TraceEntity_Ignore );
