...
void CClientFireaxe::Precache(void)
{
	m_usFireScript = PRECACHE_EVENT(1, "events/ckf_melee.sc");
}

BOOL CClientFireaxe::Deploy(void)
{
	m_iShotsFired = 0;

	return GroupDeploy("models/CKF_III/v_fireaxe.mdl", "models/CKF_III/wp_group_rf.mdl", FIREAXE_DRAW, 0, 0, "knife");
}

void CClientFireaxe::Holster(void)
{
	m_bMeleeAttack = FALSE;
}

void CClientFireaxe::PrimaryAttack(void)
{
	m_flMeleeAttack = UTIL_WeaponTimeBase() + 0.3;
	m_iMeleeCrit = g_Player.GetCriticalFire(2, g_Player.random_seed);
	m_bMeleeAttack = TRUE;
	SendWeaponAnim(FIREAXE_SLASH1 + (m_iShotsFired++) % 2);
	m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 0.8;
}

void CClientFireaxe::Swing(void)
{
