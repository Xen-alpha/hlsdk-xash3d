...
LINK_ENTITY_TO_CLASS(weapon_fireaxe, CFireaxe);
...
{
	PRECACHE_MODEL("models/CKF_III/v_fireaxe.mdl");
	PRECACHE_SOUND("CKF_III/cbar_hit1.wav");
	PRECACHE_SOUND("CKF_III/cbar_hit2.wav");
	PRECACHE_SOUND("CKF_III/fireaxe_hitbod.wav");
...
int CFireaxe::GetItemInfo(ItemInfo *p)
{
	...
	p->iFlags = 0;
	p->iWeight = FIREAXE_WEIGHT;

	return 1;
}

BOOL CFireaxe::Deploy(void)
{
	m_iShotsFired = 0;

	return GroupDeploy("models/CKF_III/v_fireaxe.mdl", "models/CKF_III/wp_group_rf.mdl", FIREAXE_DRAW, 0, 0, "knife", 63);
}

void CFireaxe::Holster(int skiplocal)
{
	m_bMeleeAttack = FALSE;
}

void CFireaxe::PrimaryAttack(void)
{
	m_flMeleeAttack = UTIL_WeaponTimeBase() + 0.3;
	m_iMeleeCrit = m_pPlayer->GetCriticalFire(2, m_pPlayer->random_seed);
	m_bMeleeAttack = TRUE;
	SendWeaponAnim(FIREAXE_SLASH1 + (m_iShotsFired++) % 2, UseDecrement() != FALSE);
	m_pPlayer->SetAnimation(PLAYER_ATTACK1);
	m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 0.8;
}

void CFireaxe::Swing(void)
{
	...
	if (tr.flFraction < 1)
	{
		...
		if(pEntity)
		{
			...
			{
				EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_STATIC, "CKF_III/fireaxe_hitbod.wav", VOL_NORM, ATTN_NORM);
		...
		if (fHitWorld)
		{
			...
			EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_STATIC, RANDOM_LONG(0, 1) ? "CKF_III/cbar_hit1.wav" : "CKF_III/cbar_hit2.wav", VOL_NORM, ATTN_NORM, 0, 98 + RANDOM_LONG(0, 3));
		}
